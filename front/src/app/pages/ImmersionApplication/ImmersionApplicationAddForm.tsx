...
import { useMakeAddImmersionApplicationUseCase } from "src/app/pages/ImmersionApplication/immersionApplication.hooks";
...
import { immersionApplicationSelectors } from "src/core-logic/domain/immersionApplication/immersionApplication.selectors";
...
export const ImmersionApplicationAddForm = ({
  ...
  const addImmersionApplicationUseCase: (
    values: ImmersionApplicationPresentation,
  ) => void = useMakeAddImmersionApplicationUseCase();

  const [initialValues] = useState(properties);

  const { enableInseeApi: inseeApiIsEnabled } =
    useAppSelector(featureFlagsSelector);

  const submitFeedback: SuccessFeedbackKind | Error | null = useAppSelector(
    immersionApplicationSelectors.submitFeedbackStatus,
  );
  ...
  return (
    <>
      ...
      <Formik
        ...
        onSubmit={addImmersionApplicationUseCase}
      >
